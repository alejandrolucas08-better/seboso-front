...
import { listCatalogByStore, getBookById } from "../../../services/book.service";
...
import { BookOpen, ArrowLeft, Store as StoreIcon, ImageOff } from "lucide-react";
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-xs flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                {catalog.length} {catalog.length === 1 ? "livro encontrado" : "livros encontrados"}
              </span>
            </div>
            ...
            {catalog.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                {catalog.map((item) => {
                  ...
                  const bookTitle = item.bookDetails?.title || `Livro #${item.book_id}`;
                  const bookAuthor = item.bookDetails?.author || "Autor não informado";
                    ...
                    >
                      ...
                      <div className="relative aspect-4/5 bg-gray-50 flex items-center justify-center border-b border-gray-100 overflow-hidden shrink-0">
                        {bookCover ? (
                          ...
                          <div className="flex flex-col items-center gap-2 text-gray-300">
                            ...
                            <span className="text-[10px] font-medium tracking-wide uppercase">Sem Capa</span>
                          </div>
                        )}

                        {item.description && (
                          <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-xs text-gray-700 text-[10px] font-bold px-2 py-1 rounded-md shadow-xs border border-gray-100 uppercase tracking-wide max-w-30 truncate" title={item.description}>
                            {item.description}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="p-4 flex flex-col flex-1 justify-between gap-3">
                        <div className="space-y-1">
                          <h3 className="font-bold text-gray-900 text-sm line-clamp-2 leading-snug group-hover:text-[#C37351] transition-colors" title={bookTitle}>
                            {bookTitle}
                          </h3>
                          <p className="text-xs text-gray-400 font-medium truncate" title={bookAuthor}>
                            {bookAuthor}
                          </p>
                        </div>
                        ...
                        <div className="pt-3 border-t border-gray-50 flex items-center justify-between mt-auto">
                          <div>
                            <span className="text-[10px] font-bold text-gray-400 block uppercase tracking-wider leading-none">Preço</span>
                          ...
                          <div className="text-right">
                            <span className="text-[9px] font-bold text-gray-400 block uppercase tracking-wider leading-none">Disponível</span>
